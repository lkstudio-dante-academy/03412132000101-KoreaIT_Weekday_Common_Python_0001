...
이러한 링크들을 통해 웹 사이트가 형성된다. (+ 즉, 하나의 화면을 구성하기 위해 여러 웹 페이지가 사용 된다는
것을 알 수 있다.)
...
def start(args):
	...
	- 특정 규칙을 지니고 있는 문자열을 탐색하기 위한 수단을 의미한다. (+ 즉, 정규 표현식을 활용하면
	문자열 내에 특정 패턴을 지니고 있는 문자열을 탐색하는 것이 가능하다.)
	"""
	...
	"""
	oListTagsA = oBSoup.findAll("img", { "src" : oCompile })
