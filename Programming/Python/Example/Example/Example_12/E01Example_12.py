...
"""
아래와 같이 다른 모듈에 존재하는 함수나 클래스에 접근하기 위해서는 from ~ import 키워드를 활용해서 모듈 내부에
존재하는 클래스를 가져오면 된다. (+ 즉, from ~ import 키워드를 통해 해당 모듈 내부에 존재하는 함수나 클래스를
사용하겠다는 것을 인터프리터에게 알려야한다.)
"""
from Example.Example_12.CCharacter import CCharacter
...
클래스는 객체 지향 프로그래밍의 핵심이 되는 사물 (객체) 을 표현하기 위한 수단으로 활용된다.
...
반환된다. (+ 즉, oSomeObj 변수에는 CSomeClass 를 통해 생성 된 객체의 참조 값이 할당 된다는 것을 알 수 있다.)
"""
