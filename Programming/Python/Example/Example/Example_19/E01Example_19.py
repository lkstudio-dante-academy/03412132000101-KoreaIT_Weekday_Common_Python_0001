...
from Example.Example_19.CList_Array import CList_Array
from Example.Example_19.CList_Linked import CList_Linked
...
자료구조는 크게 기본 자료구조와 복합 자료구조로 구분된다.
...
리스트는 구현하는 방식에 따라 배열 기반 리스트 (Array List) 와 연결 기반 리스트 (Linked List) 로 구분된다.
...
데이터를 추가하거나 제거 할 경우 데이터의 이동이 발생한다는 단점이 존재한다. (+ 즉, 데이터의 추가/삭제 가
...
발생하지 않는다는 장점이 존재한다. (+ 즉, 데이터의 추가/삭제 가 배열 기반 리스트에 비해 용이하다는 것을
알 수 있다.)
"""
