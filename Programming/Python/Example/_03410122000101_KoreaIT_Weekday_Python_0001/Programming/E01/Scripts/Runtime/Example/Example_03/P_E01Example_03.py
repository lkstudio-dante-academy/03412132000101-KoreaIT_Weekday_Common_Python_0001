...
"""
연산자 (Operator) 란?
- 데이터나 프로그램의 흐름을 제어하기 위한 특별한 의미를 지니는 기호 (심볼) 을 의미한다. (+ 즉, 연산자를
활용하면 데이터를 프로그램의 목적에 맞게 처리하는 것이 가능하다.)
...
def start(args):
	...
	"""
	컴퓨터는 0 을 제외한 모든 수를 참으로 인식하지만 Python 은 참 or 거짓을 의미하는
	True or False 데이터를 제공한다. (+ 즉, Python 에서 거짓을 나타내기 위해서는
	0 or False 를 명시하면 된다는 것을 알 수 있다.)
	"""
	print("\n=====> 관계 연산자 <=====")
	print(f"{nValA} < {nValB} = {nValA < nValB}")
	print(f"{nValA} > {nValB} = {nValA > nValB}")
	...
	"""
	컴퓨터는 음수에 대한 계산을 단순화시키기 위해서 음수를 2 의 보수법으로 표현한다. (+ 즉, 음수를
	2 의 보수법으로 표현함으로서 덧셈 연산을 통해 간단하게 음수에 대한 결과를 계산하는 것이 가능하다.)
